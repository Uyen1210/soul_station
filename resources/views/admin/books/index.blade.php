...
@section('content')
    <div class="mb-6 flex justify-between items-center">
        ...
        <a href="{{ route('admin.books.create') }}"
            class="bg-amber-600 hover:bg-amber-700 text-white font-bold py-2 px-4 rounded shadow">
            + Thêm sách mới
        </a>
    </div>
                ...
                @forelse($books as $book)
                    <tr>
                        <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                            @if($book->cover_image)
                                {{-- Kiểm tra: Nếu link bắt đầu bằng 'http' (link online) thì giữ nguyên --}}
                                <img src="{{ Str::startsWith($book->cover_image, 'http') ? $book->cover_image : asset($book->cover_image) }}"
                                    alt="{{ $book->title }}" class="w-12 h-16 object-cover rounded">
                            @else
                                <div class="w-12 h-16 bg-gray-200 flex items-center justify-center text-gray-400 text-xs">No IMG
                                </div>
                            @endif
                        </td>
