...
<body class="bg-[#f5f5dc] p-10 font-sans">
    ...
    <div class="flex justify-between items-center mb-8 border-b border-amber-200 pb-4">
        ...
        <div class="flex items-center gap-4">
            @if(Auth::check())
                ...
                <a href="{{ route('profile') }}" class="text-amber-700 font-bold hover:underline">
                    Thông tin cá nhân
                </a>
                <span class="text-gray-300">|</span>
                {{-- ================================= --}}
                ...
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <button type="submit"
                        class="text-red-500 font-bold hover:underline text-sm border border-red-200 px-3 py-1 rounded hover:bg-red-50 ml-2">
                        Đăng xuất
                    </button>
                </form>
            @else
                ...
                <a href="{{ route('register') }}"
                    class="bg-amber-600 text-white px-4 py-2 rounded font-bold hover:bg-amber-700 shadow-sm transition">
                    Đăng ký
                </a>
            @endif
        </div>
    ...
    <form class="mb-8 flex gap-2 max-w-2xl mx-auto" action="{{ route('home') }}" method="GET">
        <input type="text" name="search" value="{{ request('search') }}" placeholder="Nhập tên sách hoặc tác giả..."
            ...
            <div class="bg-white p-4 rounded-xl shadow-sm hover:shadow-md border border-amber-100 transition duration-300">
                ...
                <div class="flex justify-between items-end mt-4">
                    ...
                    <a href="{{ route('book.detail', $book->id) }}"
                        class="bg-amber-100 text-amber-800 px-3 py-1 rounded text-sm font-bold hover:bg-amber-200">
                        Chi tiết
                    </a>
                </div>
            </div>
